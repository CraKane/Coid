...
class EntityAttentionFFAgent(nn.Module):
    ...
    def forward(self, inputs, hidden_state, ret_attn_logits=None):
        if len(inputs['basic']) == 3:
            entities, obs_mask, entity_mask = inputs['basic']
        else:
            entities, obs_mask, entity_mask, gt_mask = inputs['basic']
        ...
        agent_mask = agent_mask.to(th.bool)
        ...
        if ret_attn_logits is not None:
            ...
            x2 = attn_outs
        if self.use_coach and 'coach_z' in inputs:
            coach_z = inputs['coach_z'].reshape(bs*ts, self.args.n_agents, -1)
            x2 += coach_z
        q = self.fc2(x2)
...
class ImagineEntityAttentionFFAgent(EntityAttentionFFAgent):
    ...
    def forward(self, inputs, hidden_state, imagine=False, use_gt_factors=False, use_rand_gt_factors=False):
        ...
        inputs = {"basic": (entities, obs_mask, entity_mask)}
