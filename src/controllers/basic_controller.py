...
from modules.agents.coid import Coach as Coid
...
class BasicMAC:
    ...
    def select_actions(self, ep_batch, t_ep, t_env, bs=slice(None), test_mode=False, ret_agent_outs=False):
        ...
        agent_outputs = self.forward(ep_batch, t_ep, test_mode=test_mode, acting=True)
        chosen_actions = self.action_selector.select_action(agent_outputs[bs], avail_actions[bs], t_env, test_mode=test_mode)
        if ret_agent_outs:
            return chosen_actions, agent_outputs[bs]
        if self.use_coach:
            return chosen_actions, self.coach_z, agent_outputs
        return chosen_actions

    def forward(self, ep_batch, t, coach_z=None, acting=False, test_mode=False, **kwargs):
        if t is None:
        ...
        basic_agent_inputs = self._build_inputs(ep_batch, t)
        agent_inputs = {'basic': basic_agent_inputs}
