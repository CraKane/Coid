...
class ParallelRunner:
    ...
    def reset(self, **kwargs):
        ...
        if self.use_coach:
            ...
            if self.coid:
                n_env = len(pre_transition_data['entities'])
                self.no_terminated = list(range(n_env))
                pre_transition_data.update({ 'pre_actions':th.zeros(n_env, self.args.n_agents, self.args.n_actions).to(self.args.device)})
                pre_transition_data.update({ 'pre_role':th.zeros(n_env, self.args.n_agents, self.args.rnn_hidden_dim).to(self.args.device)})
    ...
    def run(self, test_mode=False, test_scen=None, index=None, vid_writer=None):
        ...
        while True:
            ...
            if self.use_coach:
                actions, z_team, action_outs = self.mac.select_actions(self.batch, t_ep=self.t, t_env=self.t_env, bs=envs_not_terminated, test_mode=test_mode)
            ...
            if self.use_coach:
                pre_transition_data["hier_decision"] = []
            ...
            for idx, parent_conn in enumerate(self.parent_conns):
                if not terminated[idx]:
                    ...
                    if self.use_coach:
                        hier_timer[idx] += 1
                        if hier_timer[idx] >= self.comm_interval and not terminated[idx]:
                            hier_timer[idx] = 0
            ...
            if self.coid:
                # Only store data in the activate env
                pre_chosen = {
                    "pre_actions": action_outs[self.no_terminated],
                    "pre_role": z_team[self.no_terminated]
                }
                pre_transition_data.update(pre_chosen)

            self.no_terminated = np.where(np.array(post_transition_data['terminated'])<1)[0]
